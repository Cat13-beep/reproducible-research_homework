...
I can use the predicted values of alpha and beta from my answer to 5c to predict the volume of a virion with a genomic size of 300kb for a double-stranded DNA virus. As discussed above my values are the same as that of the double-stranded DNA values in the table so I can be confident in my estimated answers for values for alpha and beta. Inputting these into the equation V=aL^b (in question it states L is nucleotides which suggests having to multiply 300 by 1000 however in the dataset and graph kb are used so that is what I will put into my equation here) which is 1181.807 x 300^1.5152= a viral volume of 6.697x10^6 nm^3.
